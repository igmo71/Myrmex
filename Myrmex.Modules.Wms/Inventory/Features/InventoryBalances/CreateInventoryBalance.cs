...
internal static class CreateInventoryBalance
{
        ...
        : ICommandHandler<Command, ServiceResult<InventoryBalanceDetails>>
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            InventoryBalanceDetails? details = await ProjectDetails()
                .SingleOrDefaultAsync(
                    x => x.Id == inventoryBalance.Id,
                    cancellationToken);
        ...
        private IQueryable<InventoryBalanceDetails> ProjectDetails()
        {
            return from inventoryBalance in dbContext.InventoryBalances.AsNoTracking()
                   join stockKeepingUnit in dbContext.StockKeepingUnits.AsNoTracking()
                       on inventoryBalance.StockKeepingUnitId equals stockKeepingUnit.Id
                   join storageLocation in dbContext.StorageLocations.AsNoTracking()
                       on inventoryBalance.StorageLocationId equals storageLocation.Id
                   join warehouse in dbContext.Warehouses.AsNoTracking()
                       on storageLocation.WarehouseId equals warehouse.Id
                   join baseUnitOfMeasure in dbContext.UnitsOfMeasure.AsNoTracking()
                       on stockKeepingUnit.BaseUnitOfMeasureId equals baseUnitOfMeasure.Id
                   select new InventoryBalanceDetails(
                       inventoryBalance.Id,
                       ...
                       inventoryBalance.CreatedAtUtc,
                       inventoryBalance.UpdatedAtUtc);
        }
    }
}
