...
internal static class PickInventoryTransferLine
{
        ...
        : ICommandHandler<Command, ServiceResult<InventoryTransferDetails>>
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            decimal transitBalanceBefore = transitBalance?.Quantity ?? 0;
            ...
            if (transitBalance is null)
            {
                DomainValidationResult transitBalanceCreateValidation = InventoryBalance.Create(
                    line.StockKeepingUnitId,
                    transitStorageLocationId,
                    transitBalanceAfter,
                    out transitBalance);

                if (!transitBalanceCreateValidation.IsValid)
                {
                    return ServiceResult<InventoryTransferDetails>.Invalid(transitBalanceCreateValidation.Errors);
                }

                dbContext.InventoryBalances.Add(transitBalance!);
            }
            else
            {
            ...
            decimal quantity)
        {
            ...
            {
                return PickRequiresTransitConflict();
            }

            decimal pickedQuantity = line.GetPickedQuantity(transfer.Movements);

            if (pickedQuantity + quantity > line.RequestedQuantity)
            {
                return OverPickConflict();
            }
    ...
    internal static ServiceError CompletedTransferConflict()
    {
        return new ServiceError(
            ...
            "Completed inventory transfers cannot be picked.",
            nameof(Command.TransferId));
    }

    internal static ServiceError PickRequiresTransitConflict()
    {
        return new ServiceError(
            ServiceErrorType.Conflict,
            "InventoryTransfer.PickRequiresTransitTransfer",
            "Pick is only available for transfers with a transit storage location.",
            nameof(Command.TransferId));
    }

    internal static ServiceError OverPickConflict()
    {
        return new ServiceError(
            ServiceErrorType.Conflict,
            "InventoryTransfer.OverPick",
            "Pick quantity exceeds the remaining requested quantity.",
    ...
    internal static ServiceError InsufficientSourceBalanceConflict()
    {
        return new ServiceError(
            ...
            "Source storage location does not have enough available inventory for the pick.",
            nameof(Command.Quantity));
    }
    ...
    internal static ServiceError MovementConcurrencyConflict()
    {
        return new ServiceError(
            ...
            "Inventory balances changed while the transfer pick was being saved. Refresh and try again.",
            nameof(Command.TransferId));
    }
}
