...
internal static class CompleteReceivingOrder
{
        ...
        : ICommandHandler<Command, ServiceResult<ReceivingOrderDetails>>
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (order.Status != ReceivingOrderStatus.InProgress || !order.IsFullyReceived)
            {
                return ServiceResult<ReceivingOrderDetails>.Fail(
            ...
            Guid[] skuIds = order.Lines.Select(x => x.StockKeepingUnitId).ToArray();
            ...
            foreach (ReceivingOrderLine line in order.Lines)
            {
                decimal before = existingBalances.TryGetValue(line.StockKeepingUnitId, out InventoryBalance? balance)
                    ? balance.Quantity
                    : 0;
                decimal after;
                try
