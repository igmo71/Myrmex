...
Phase 1 contracts and data model preserve the same boundaries and contain no constitution exception. The additive migration is explicitly constrained against destructive legacy-column/index operations. Cancellation uses the existing Processing/abandoned-recovery model while propagating to the caller. No Complexity Tracking entry is required.
...
└── Infrastructure/Persistence/
    ...
    ├── Migrations/<prepared AddReferenceExternalDataVersion migration source>.cs
    └── Migrations/WmsDbContextModelSnapshot.cs
...
- **Persistence and migration**: Configure `ExternalImportState` as an optional owned reference with backing-field access for the binary version and exact same-table column names. Prepare and review migration source/model-snapshot changes so only three nullable `ExternalDataVersion` columns are added/dropped; preserve legacy values and filtered unique indexes exactly. Do not plan migration-generation or database-update command execution.
...
- **Implementation/execution boundary**: Implementation may prepare source, test, EF mapping, migration source, and model-snapshot changes. This plan and later tasks do not schedule build, test, migration-generation, database-update, AppHost, Docker, application-startup, or other environment-changing command execution; those operations remain developer-controlled and require an explicit user request.
...
4. Prepare and review WMS migration source/model-snapshot changes; reject destructive legacy-column or index operations without executing migration-generation or database-update commands.
...
| Exact owned columns, nullable version, legacy indexes | One parameterized EF metadata/provider-sensitive persistence test across three entity types plus migration operation review | Do not duplicate general uniqueness tests already present. |
