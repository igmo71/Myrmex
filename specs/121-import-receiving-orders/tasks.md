...
- [ ] T002 Configure the Warehouse default receiving-location FK and validate the referenced Receiving location in `Myrmex.Modules.Wms/Infrastructure/Persistence/Configurations/WarehouseConfiguration.cs` and `Myrmex.Modules.Wms/Topology/Features/Warehouses/UpdateWarehouseDetails.cs`.
...
- [ ] T004 Update the existing Warehouse PUT command mapping and returned details projection in `Myrmex.Modules.Wms/Topology/Endpoints/WarehouseEndpoints.cs` and `Myrmex.Modules.Wms/Topology/Features/Warehouses/WarehouseDetails.cs`.
...
- [ ] T006 Add ReceivingOrder-owned external import-state behavior that records the opaque 1C document key/version only after a successful import outcome in `Myrmex.Modules.Wms/Receiving/Domain/ReceivingOrders/ReceivingOrder.cs`.
...
- [ ] T010 [US1] Implement typed `Document_ПриходныйОрдерНаТовары` and `Товары` DTOs plus source-period OData reading, deterministic Date/Ref_Key ordering, posted/deletion/status eligibility filtering, and source-neutral mapping in `Myrmex.Integrations/OneC/ReceivingOrders/ReceivingOrderSourceRecord.cs`, `Myrmex.Integrations/OneC/ReceivingOrders/ReceivingOrderSourceLineRecord.cs`, and `Myrmex.Integrations/OneC/ReceivingOrders/ReceivingOrderOneCSource.cs`.
- [ ] T011 [US1] Validate external document/line identities, map `Количество` to planned quantity, retain `КоличествоУпаковок` only for diagnostics, and return `UnsupportedPackage` for a non-empty `Упаковка_Key` in `Myrmex.Integrations/OneC/ReceivingOrders/ReceivingOrderOneCMapper.cs`.
...
- [ ] T013 [US1] Implement the public source-neutral single-document import command to resolve imported Warehouse/SKU dependencies, derive and validate the Warehouse default Receiving location, aggregate lines by SKU, create a Draft order, and return a structured outcome in `Myrmex.Modules.Wms/Receiving/Features/ReceivingOrders/ImportExternalReceivingOrder.cs`.
- [ ] T014 [US1] Use one WMS transaction per document, map domain/persistence/dependency failures to stable reasons, and emit structured Created/Skipped/Failed diagnostics in `Myrmex.Modules.Wms/Receiving/Features/ReceivingOrders/ImportExternalReceivingOrder.cs`.
- [ ] T015 [US1] Implement the scoped manual import operation that reads the selected period, maps every document, dispatches one WMS import command per document, and accumulates operator results in `Myrmex.Integrations/OneC/ReceivingOrders/ReceivingOrderOneCImport.cs`.
...
- [ ] T025 [US3] Display document identity, immediate failure/skip reason, and request-wide transport/configuration errors distinctly in `Myrmex.WebApp/Components/Pages/Integrations/OneC/Index.razor` and `Myrmex.WebApp/Resources/Localization/SharedResource.ru-RU.resx`.
