...
- **Health/readiness**: ApiService already calls `builder.AddServiceDefaults()` and `app.MapDefaultEndpoints()`. `Myrmex.ServiceDefaults` maps `/health` as the readiness endpoint and `/alive` as the liveness endpoint in Development/Staging; AppHost already uses `.WithHttpHealthCheck("/health")` for ApiService. The integration slice must reuse `/health` by registering only an `IntegrationDbContext` persistence reachability check from `AddOneCIntegration`; `/alive` remains the platform self liveness check and does not depend on integration SQL availability. Required API-key, source identity, polling, timeout, batch, and retry configuration remains covered by startup options validation, not duplicated in health checks. Worker registration, loop entry, startup scan, polling, wake-up handling, cancellation, retry, and abandoned-processing recovery remain covered by worker/lifecycle tests, not by health checks. Health output must not expose API keys, connection strings, external credentials, queue contents, synchronization-request details, or internal exception details. No additional readiness infrastructure or public endpoint is introduced.
- **Risk-based testing**: Protect exact JSON field binding through explicit JSON property mapping, required fields, GUID `Ref_Key` validation, non-empty bounded `DataVersion` decoding, field-identifying ProblemDetails-style validation responses, malformed Date rejection, unknown-property tolerance, API-key auth separation, existing WMS operator route preservation, durable uniqueness, duplicate lifecycle preservation, Date diagnostic handling, retry/lifecycle transitions including valid empty `RetryDelaysSeconds` with one attempt and no retries, processing-attempt timeout versus host-shutdown cancellation, startup scan, coalescing wake-up fallback, and abandoned `Processing` recovery. Health/readiness is covered as implementation and manual quickstart validation scope, not additional automated test scope for this issue. Omit WebApp/API-client tests because no client/UI is added.
