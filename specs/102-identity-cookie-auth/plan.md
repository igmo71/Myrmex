...
- **Development and test authentication**: The temporary development/staging actor authentication path is removed. Test authentication remains a separate test-only scheme whose options explicitly control stable GUID actor ID and roles. It is used only by tests and cannot be selected as a production fallback.
