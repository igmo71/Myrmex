...
- [X] T007 [P] [US1] After T010 creates `WarehouseOneCSource`, retarget only the existing Warehouse projection, optional folder filtering, key lookup, current-object cardinality/DataVersion shape, fallback-code shape, and Warehouse folder behavior in `Myrmex.Tests/Integrations/OneC/Client/OneCODataClientTests.cs`, leaving common transport coverage in T006 and adding no Warehouse matrix; this task depends on T010.
...
- [X] T018 [US2] Define the UoM source record, exact Unicode `$select`, stable ordering, key lookup, current-object cardinality/DataVersion validation, and probe in `Myrmex.Integrations/OneC/UnitsOfMeasure/UnitOfMeasureSourceRecord.cs` and `Myrmex.Integrations/OneC/UnitsOfMeasure/UnitOfMeasureOneCSource.cs` using `IOneCODataTransport` and no folder field, filter, or outcome.
