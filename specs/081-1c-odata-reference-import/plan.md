...
- **Source mapping**: `Ref_Key -> ExternalRefKey`, `Code -> Code`, and 1C `Description -> Name`. Existing local description/symbol values are preserved when the source has no equivalent field. Optional `Артикул` remains transport-only because `StockKeepingUnit` has no article-number concept; it must not be misused as Myrmex `Description`.
- **Required SKU base UoM**: Add `DefaultSkuBaseUnitOfMeasureExternalRefKey` to `OneCOptions`. The adapter supplies it on each SKU item. WMS resolves it to one active imported UoM and reports `BaseUnitOfMeasureNotFound` or `BaseUnitOfMeasureInactive` per record. UoMs must be imported before SKUs; code matching is prohibited.
...
- **OData paging**: Nomenclature requests use `$format=json`, `$orderby=Ref_Key`, `$skip`, `$top`, and `$select`. Offset advances by returned count; paging stops when fewer than `BatchSize` records return, including a final empty request after an exact-size batch. Warehouses and UoMs use separate non-paged reads with explicit `$orderby` and `$select`.
...
- **Risk-based testing**: WMS tests own idempotency, source/code conflicts, lifecycle, base-UoM resolution, filtered indexes, rollback, and committed counts. OneC client tests own exact queries, Unicode DTO deserialization, paging, timeout, and malformed/upstream responses. Orchestrator tests own mapping, aggregation, partial failure, cancellation, and locking. Focused endpoint tests own routes, 401/409/200 serialization; WebApp client tests own route and ProblemDetails mapping. UI uses manual smoke validation.
