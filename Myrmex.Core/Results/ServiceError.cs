    ...
    IReadOnlyList<ServiceError>? Details = null)
{
    ...
    public static ServiceError NotFound<TEntity>(string? message = null, string? property = null)
        => new(ServiceErrorType.NotFound, $"NotFound-{typeof(TEntity).Name}-{property}".TrimEnd('-'), message ?? $"{typeof(TEntity).Name} not found.", property);

    public static ServiceError Conflict<TEntity>(string? message = null, string? property = null)
        => new(ServiceErrorType.Conflict, $"Conflict-{typeof(TEntity).Name}-{property}".TrimEnd('-'), message ?? $"{typeof(TEntity).Name} is in conflict.", property);

    public static ServiceError Validation<TEntity>(string? message = null, string? property = null)
        => new(ServiceErrorType.Invalid, $"Validation-{typeof(TEntity).Name}-{property}".TrimEnd('-'), message ?? $"{typeof(TEntity).Name} has validation errors.", property);
    ...
    public static ServiceError Failure<TEntity>(string message, string? property = null)
        => new(ServiceErrorType.Failure, $"Failure-{typeof(TEntity).Name}-{property}".TrimEnd('-'), message, property);
}